...
const isPalAdStandardPlanCode = (code: string): boolean => {
  const normalized = normalize(code).replace(/-/g, '_');
  return normalized === 'pal_ad_standard';
};
...
export const listPalAdAccountsFromPalDb = async (): Promise<PalDbAccount[]> => {
  ...
  const targetPlanIds = new Set(
    plans
      .filter((plan) => isPalAdPlanCode(plan.code))
  ...
  const standardPlanIds = new Set(
    plans
      .filter((plan) => isPalAdStandardPlanCode(plan.code))
      .map((plan) => String(plan.id || '').trim())
      .filter(Boolean),
  );
...
export const canLoginPalAdByPaletteId = async (paletteId: string): Promise<boolean> => {
  ...
  return contracts.some((item) => {
    const accountId = String(item.accountId || '').trim();
    const planId = String(item.planId || '').trim();
    return accountId === String(account.id || '').trim() && adPlanIds.has(planId);
  });
};
